// Taken from https://github.com/nem0/LumixEngine and modified by removing dependencies on LumixEngine file objects and Lua
// @NOTE: License? 
...
int vmajor_current = 1;
int vminor_current = 2;
int vpatch_current = 1;
...
// @NOTE(mv): For some reason it complains about these operators and the assert define
...
struct DockContext
{
	...
	// hacks
	bool isDockDocked(const char* label) {
		Dock &dock = getDock(label, true, ImVec2(-1, -1));
		return dock.status == Status_Docked;
	}

	bool isNonDockedDockAt(const ImVec2& pos) {
		for (int i = 0; i < m_docks.size(); ++i)
		{
			Dock& dock = *m_docks[i];
			if (dock.hasChildren()) continue;
			if (dock.status != Status_Docked && dock.opened) {
	...
	void setTabActive(const char *label) {
		Dock &dock = getDock(label, true, ImVec2(-1,-1));
		dock.setActive();
	}
	...
	void splits()
	{
		...
		for (int i = 0; i < m_docks.size(); ++i)
		{
			...
			if (dock.isHorizontal()) {
				...
				//if (fabs(dsize.x) > 0.0) printf("%f\n", dsize.x); fflush(stdout);
	...
	{
		close_button = false; // @WARNING: always show close button
		...
		if (BeginChild(tmp, size, true))
		{
			...
			while (dock_tab)
			{
				...
				*/
				...
				bool hovered = IsItemHovered();
				ImVec2 pos = GetItemRectMin();
				size.x += 0 + GetStyle().ItemSpacing.x;
				
				tab_base = pos.y;

				draw_list->AddRectFilled(pos+ImVec2(-4.0f, 0.0),
										 pos+size,
										 hovered ? color_hovered : (dock_tab->active ? color_active : color));
				draw_list->AddText(pos, text_color, dock_tab->label, text_end);
	...
	*/
	void save()
	{
		FILE *fp = fopen("docks.layout", "w");
		fprintf(fp, "version %d.%02d.%d\n\n", 1, 2, 1);
		fprintf(fp, "docks %d\n\n", m_docks.size());
		for (int i = 0; i < m_docks.size(); ++i) {
			Dock& dock = *m_docks[i];

			fprintf(fp, "index    %d\n", i);
			fprintf(fp, "label    %s\n", dock.parent || dock.status == Status_Float ? (dock.label[0] == '\0' ? "DOCK" : dock.label) : "ROOT"),
			fprintf(fp, "x        %d\n", (int)dock.pos.x);
			fprintf(fp, "y        %d\n", (int)dock.pos.y);
			fprintf(fp, "size_x   %d\n", (int)dock.size.x);
			fprintf(fp, "size_y   %d\n", (int)dock.size.y);
	...
	void load(bool load_default = false)
	{
		//printf("%s\n", default_layout);
		for (int i = 0; i < m_docks.size(); ++i)
		{
		...
		FILE *fp = fopen("docks.layout", "r");

		int vmajor = 0, vminor = 0, vpatch = 0;
		if (fp) {
			fscanf(fp, "version %d.%02d.%d\n", &vmajor, &vminor, &vpatch);
		}
		//printf("%d %d %d\n", vmajor, vminor, vpatch); fflush(stdout);

		if ((vmajor == vmajor_current && vminor == vminor_current && vpatch == vpatch_current) && fp && !load_default) {
			int ival = 0;
			char str2[64];
		...
		//	printf("ival = %d\n", ival); fflush(stdout);
			...
			sscanf(default_layout[ctr++], "version %d.%02d.%d", &vmajor, &vminor, &vpatch);
			sscanf(default_layout[ctr++], "docks %d", &ival);
			//printf("vmajor = %d, vminor = %d, vpatch = %d, ival = %d\n", vmajor, vminor, vpatch, ival); fflush(stdout);
			ctr++;
			...
			for (int i = 0; i < ival; i++) {
				...
				sscanf(default_layout[ctr++], "%s %d", str2, &id);
				sscanf(default_layout[ctr++], "%s %[^\n]s", str2, &lab[0]);
				sscanf(default_layout[ctr++], "%s %f", str2, &m_docks[id]->pos.x);
				sscanf(default_layout[ctr++], "%s %f", str2, &m_docks[id]->pos.y);
				sscanf(default_layout[ctr++], "%s %f", str2, &m_docks[id]->size.x);
				sscanf(default_layout[ctr++], "%s %f", str2, &m_docks[id]->size.y);
				sscanf(default_layout[ctr++], "%s %d", str2, &st);
				sscanf(default_layout[ctr++], "%s %d", str2, &b1);
				sscanf(default_layout[ctr++], "%s %d", str2, &b2);
				sscanf(default_layout[ctr++], "%s %s", str2, &m_docks[id]->location[0]);
				sscanf(default_layout[ctr++], "%s %d", str2, &id1);
				sscanf(default_layout[ctr++], "%s %d", str2, &id2);
				sscanf(default_layout[ctr++], "%s %d", str2, &id3);
				sscanf(default_layout[ctr++], "%s %d", str2, &id4);
				sscanf(default_layout[ctr++], "%s %d", str2, &id5);
				ctr++;
...
void LoadDock(bool load_default = false)
{
	g_dock.load(load_default);
}

bool isDockDocked(const char* label) {
	return g_dock.isDockDocked(label);
}

bool isNonDockedDockAt(const ImVec2& pos) {
	return g_dock.isNonDockedDockAt(pos);
}


void setTabActive(const char *label) {
	g_dock.setTabActive(label);
}
